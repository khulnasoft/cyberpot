...
VERSION="CyberPot $(cat /opt/cyberpot/version)"
...
if [ "${myOSTYPE}" == "linuxkit" ] && [ "${CYBERPOT_OSTYPE}" == "linux" ];
  then
...
if [ -f "/data/uuid" ];
  then
    ...
    mkdir -vp /data/ews/conf \
              ...
              /data/cyberpot/etc/compose/ \
	      /data/cyberpot/etc/logrotate/ \
              /tmp/etc/
    echo
    ...
    echo
    echo "# Creating web user from cyberpot.env, make sure to erase the password from the .env ..."
    echo
