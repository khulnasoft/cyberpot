#!/bin/bash
# CyberPot Universal Installer

# Installer can only be executed once.
myCYBERPOT_INSTALL_LOG="/install.log"
if [ -s "$myCYBERPOT_INSTALL_LOG" ];
  then
...
myBACKTITLE="CyberPot-Installer"
...
myCYBERPOTCOMPOSE="/opt/cyberpot/etc/cyberpot.yml"
...
elif [ -f "/opt/cyberpot/packages.txt" ];
  then myINSTALLPACKAGESFILE="/opt/cyberpot/packages.txt"
elif [ -f "/root/cyberpot/packages.txt" ];
  then myINSTALLPACKAGESFILE="/root/cyberpot/packages.txt"
else
...
myINFO="\
###########################################
### CyberPot Installer for Debian (Stable) ###
###########################################

Disclaimer:
This script will install CyberPot on this system.
...
$myRANDOM_MINUTE $myPULL_HOUR * * *      root    docker-compose -f /opt/cyberpot/etc/cyberpot.yml pull

# Uploaded binaries are not supposed to be downloaded
*/1 * * * *     root    mv --backup=numbered /data/dionaea/roots/ftp/* /data/dionaea/binaries/

# Daily reboot
$myRANDOM_MINUTE $myRANDOM_HOUR * * 1-6      root    systemctl stop cyberpot && docker stop \$(docker ps -aq) && docker rm \$(docker ps -aq); reboot
...
function fuCHECKNET {
  if [ "$myCYBERPOT_DEPLOYMENT_TYPE" == "iso" ] || [ "$myCYBERPOT_DEPLOYMENT_TYPE" == "user" ];
    then
      local mySITES="$1"
...
function fuGET_DEPS {
  ...
  echo
  echo "### Installing CyberPot dependencies."
  echo
...
if [ "$myCYBERPOT_DEPLOYMENT_TYPE" == "user" ];
  then
    echo
    ...
    echo "### might collide with CyberPot's honeypots and prevent CyberPot from starting successfully."
    echo
    while [ 1 != 2 ]
      do
...
for i in "$@"
  do
    case $i in
      --conf=*)
        myCYBERPOT_CONF_FILE="${i#*=}"
        shift
      ;;
      --type=user)
        myCYBERPOT_DEPLOYMENT_TYPE="${i#*=}"
        shift
      ;;
      --type=auto)
        myCYBERPOT_DEPLOYMENT_TYPE="${i#*=}"
	...
	echo "  user, use this if you want to manually install a CyberPot on a Debian (Stable) machine."
        ...
        echo "  iso, use this if you are a CyberPot developer and want to install a CyberPot from a pre-compiled iso."
        echo
	exit
      ;;
      *)
...
if [ "$myCYBERPOT_DEPLOYMENT_TYPE" == "auto" ] && [ "$myCYBERPOT_CONF_FILE" == "" ];
  then
    echo "Aborting. No configuration file given."
    exit
fi
if [ -s "$myCYBERPOT_CONF_FILE" ] && [ "$myCYBERPOT_CONF_FILE" != "" ];
  then
    myCYBERPOT_DEPLOYMENT_TYPE="auto"
    if [ "$(head -n 1 $myCYBERPOT_CONF_FILE | grep -c "# cyberpot")" == "1" ];
      then
        source "$myCYBERPOT_CONF_FILE"
      else
	echo "Aborting. Config file \"$myCYBERPOT_CONF_FILE\" not a CyberPot configuration file."
        exit
      fi
  elif ! [ -s "$myCYBERPOT_CONF_FILE" ] && [ "$myCYBERPOT_CONF_FILE" != "" ];
    then
      echo "Aborting. Config file \"$myCYBERPOT_CONF_FILE\" not found."
      exit
fi

# Prepare running the installer
myUSERCHECK=$(grep "cyberpot" /etc/passwd | wc -l)
if [ "$myUSERCHECK" -gt "0" ];
  then
    echo "### The user name \"cyberpot\" already exists. The cyberpot username and group may not previously exist or CyberPot will not work."
    echo "### We recommend a fresh install according to the CyberPot Readme Post-Install method."
    echo
    echo "Aborting."
    echo
    exit 0
...
if [ "$myCYBERPOT_DEPLOYMENT_TYPE" == "iso" ];
  then
    sleep 5
...
if [ "$myCYBERPOT_DEPLOYMENT_TYPE" == "iso" ] || [ "$myCYBERPOT_DEPLOYMENT_TYPE" == "user" ];
  then
    myCONF_CYBERPOT_FLAVOR=$(dialog --keep-window --no-cancel --backtitle "$myBACKTITLE" --title "[ Choose Your CyberPot Edition ]" --menu \
    "\nRequired: 8-16GB RAM, 128GB SSD\nRecommended: 16GB RAM, 256GB SSD" 17 70 1 \
    "STANDARD" "CyberPot Standalone with everything you need" \
    "HIVE" "CyberPot Hive: ELK & Tools" \
    "HIVE_SENSOR" "CyberPot Hive Sensor: Honeypots & NSM" \
...
if [ "$myCYBERPOT_DEPLOYMENT_TYPE" == "iso" ];
  then
    ...
    printf "%s" "$myCONF_CYBERPOT_USER:$myPASS1" | chpasswd
fi
...
if ! [ "$myCONF_CYBERPOT_FLAVOR" == "SENSOR" ];
  then
...
# Cloning CyberPot from GitHub
if ! [ "$myCYBERPOT_DEPLOYMENT_TYPE" == "iso" ];
  then
    fuBANNER "Cloning CyberPot"
    ### DEV
    git clone https://github.com/khulnasoft/cyberpot /opt/cyberpot
fi

# Let's create the CyberPot user
fuBANNER "Create groups"
addgroup --gid 2000 cyberpot
addgroup cyberpotlogs
fuBANNER "Create user"
adduser --system --no-create-home --uid 2000 --disabled-password --disabled-login --gid 2000 cyberpot

# Let's set the hostname
a=$(fuRANDOMWORD /opt/cyberpot/host/usr/share/dict/a.txt)
n=$(fuRANDOMWORD /opt/cyberpot/host/usr/share/dict/n.txt)
myHOST=$a$n
...
# Let's make sure only myCONF_CYBERPOT_FLAVOR images will be downloaded and started
case $myCONF_CYBERPOT_FLAVOR in
  STANDARD)
    fuBANNER "STANDARD"
    ln -s /opt/cyberpot/etc/compose/standard.yml $myCYBERPOTCOMPOSE
  ;;
  HIVE)
    fuBANNER "HIVE"
    ln -s /opt/cyberpot/etc/compose/hive.yml $myCYBERPOTCOMPOSE
  ;;
  HIVE_SENSOR)
    fuBANNER "HIVE_SENSOR"
    ln -s /opt/cyberpot/etc/compose/hive_sensor.yml $myCYBERPOTCOMPOSE
  ;;
  INDUSTRIAL)
    fuBANNER "INDUSTRIAL"
    ln -s /opt/cyberpot/etc/compose/industrial.yml $myCYBERPOTCOMPOSE
  ;;
  LOG4J)
    fuBANNER "LOG4J"
    ln -s /opt/cyberpot/etc/compose/log4j.yml $myCYBERPOTCOMPOSE
  ;;
  MEDICAL)
    fuBANNER "MEDICAL"
    ln -s /opt/cyberpot/etc/compose/medical.yml $myCYBERPOTCOMPOSE
  ;;
  MINI)
    fuBANNER "MINI"
    ln -s /opt/cyberpot/etc/compose/mini.yml $myCYBERPOTCOMPOSE
  ;;
  SENSOR)
    fuBANNER "SENSOR"
    ln -s /opt/cyberpot/etc/compose/sensor.yml $myCYBERPOTCOMPOSE
  ;;
esac
...
echo "$myFAIL2BANCONF" | tee /etc/fail2ban/jail.d/cyberpot.conf
...
cp /opt/cyberpot/host/etc/systemd/* /etc/systemd/system/
...
if [ "$myCYBERPOT_DEPLOYMENT_TYPE" == "iso" ];
  then
    usermod -a -G cyberpot tsec
    chown tsec:tsec -R /home/tsec/.ssh
  else
    usermod -a -G cyberpot $(who am i | awk '{ print $1 }')
fi
...
# Let's enable a color prompt and add /opt/cyberpot/bin to path
fuBANNER "Setup prompt"
tee -a /root/.bashrc <<EOF
$mySHELLCHECK
$myROOTPROMPT
$myROOTCOLORS
PATH="\$PATH:/opt/cyberpot/bin"
EOF
for i in $(ls -d /home/*/)
  do
tee -a $i.bashrc <<EOF
...
# Final steps
cp /opt/cyberpot/host/etc/rc.local /etc/rc.local && \
...
if [ "$myCYBERPOT_DEPLOYMENT_TYPE" == "auto" ];
  then
