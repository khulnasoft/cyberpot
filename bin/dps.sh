...
# Show current status of CyberPot containers
myCONTAINERS="$(cat /opt/cyberpot/etc/cyberpot.yml | grep -v '#' | grep container_name | cut -d: -f2 | sort | tr -d " ")"
myRED="[1;31m"
myGREEN="[1;32m"
myBLUE="[1;34m"
myWHITE="[0;0m"
...
function fuGETCYBERPOT_STATUS {
# CyberPot Status
myCYBERPOT_STATUS=$(systemctl status cyberpot | grep "Active" | awk '{ print $2 }')
if [ "$myCYBERPOT_STATUS" == "active" ];
  then
    echo "${myGREEN}ACTIVE"
  else
    echo "${myRED}INACTIVE"
