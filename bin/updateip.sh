...
myCHECKIFSENSOR=$(head -n 1 /opt/cyberpot/etc/cyberpot.yml | grep "Sensor" | wc -l)
...
# Build issue
echo "[H[2J" > /etc/issue
toilet -f ivrit -F metal --filter border:metal "CyberPot   22.04" | sed 's/\\/\\\\/g' >> /etc/issue
echo >> /etc/issue
...
ip = $myEXTIP
EOF
tee /opt/cyberpot/etc/compose/elk_environment << EOF
HONEY_UUID=$myUUID
MY_EXTIP=$myEXTIP
...
if [ -s "/data/elk/logstash/ls_environment" ];
  then
    ...
    tee -a /opt/cyberpot/etc/compose/elk_environment << EOF
